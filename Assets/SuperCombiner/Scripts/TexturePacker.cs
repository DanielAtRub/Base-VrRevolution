    ...
    public class TexturePacker  {
		...
		private Vector3 GetTextureSizeInAtlas(Vector2 inputTextureSize, float scaleX, float scaleY, string materialName) {
			...
			if (size.x >= MAX_TEXTURE_SIZE || size.y >= MAX_TEXTURE_SIZE) {
                ...
                Logger.Instance.AddLog("SuperCombiner", "Textures in _material '" + materialName + "' are being tiled and the total tiled size exceeds the maximum texture size for the current plateform (" + SystemInfo.maxTextureSize + "). All textures in this _material will be shrunk by " + reducingFactor + " to fit in the atlas. This could leads to a quality loss. Whenever possible, avoid combining tiled texture.");
			}
			return size;
		}
			...
			foreach(KeyValuePair<string, List<Texture2D>> keyValue in _texturesForAtlas) {
				if (mat.HasProperty (keyValue.Key)) {
					...
					if (keyValue.Key.Equals ("_MainTex")) {
                        ...
                        Logger.Instance.AddLog("SuperCombiner", "Found materials with properties that don't match. Maybe you are trying to combine different shaders that don't share the same properties.", Logger.LogLevel.LOG_WARNING);
					}
				}
			}
		}
