...
#pragma warning restore
		...
		Vector3 UnitVelocity () {
			...
			if ( myNavMeshAgent ) {
				...
				vel = myRigidbody.linearVelocity;
			} else if ( myRigidbody2D && myRigidbody2D.isKinematic == false ) {
				vel = myRigidbody2D.linearVelocity;
		...
		void Update () {
			...
			if ( target && requestAngle == false ) {
				...
				//Debug.DrawRay( weaponExit.position, _cross, Color.red, .01f );
				//Debug.DrawRay( weaponExit.position, _level, Color.green, .01f );
				//Debug.DrawRay( weaponExit.position, targetAimLocation - weaponExit.position, Color.cyan, .01f );
				//Debug.Log( _angle );
			...
			if ( limitLeftRight < 180 ) { // is there a gimbal limit?
				float _bearing = Vector3.SignedAngle( transform.forward, rotatorPlaneTarget - weaponExit.position, transform.up );
				float _aimAngle = Vector3.SignedAngle( transform.forward, rotator.forward, rotator.up );
			...
			Quaternion _rot;
			Vector3 _localMount = rotator.InverseTransformPoint( weaponExit.position );

			// apply rotation
			Vector3 v = rotatorPlaneTarget - rotator.TransformPoint( new Vector3( _localMount.x, 0f, _localMount.z ) );
			if ( v != Vector3.zero ) { // prevent LookRotation is 0 error
				_rot = Quaternion.LookRotation( v, transform.up );
				rotator.rotation = Quaternion.RotateTowards( rotator.rotation, _rot, _modRotRate * Time.deltaTime );
			}
			// apply elevation
			v = elevatorPlaneTarget - weaponExit.position;
			if ( v != Vector3.zero ) { // prevent LookRotation is 0 error
				_rot = Quaternion.LookRotation( v, rotator.up );
		...
		Vector3 AimLocation () {
			...
			if ( target ) {
					...
					if ( shotSpeed != 0 ) {
						...
						if ( aimType == AimType.Intercept || aimType == AimType.BallisticIntercept ) {
							if ( targetNavMeshAgent ) {
								...
								targetVelocity = targetRigidbody2D.linearVelocity;
				...
				if ( hasSolution == true ) {
					if ( ( aimType == AimType.Ballistic || aimType == AimType.BallisticIntercept ) && Physics.gravity.y != 0 ) { // ballistic aim
						...
						if ( _ballAim == null ) {
							...
							if ( aimType == AimType.BallisticIntercept ) { // ballistic + intercept, iterate for better ballistic accuracy
								if ( !Mathf.Approximately( myVelocity.sqrMagnitude, 0f ) || !Mathf.Approximately( targetVelocity.sqrMagnitude, 0f ) ) { // don't need if shooter and target are not moving 
									...
									//if ( ballisticArc == MFnum.ArcType.Low ) {
									//	biMax = lowArcIterations;
									//}
							...
							if ( hasSolution == true ) { // solution can fail in balistic iteration
								Vector3 _cross = -Vector3.Cross( ( targetLoc - weaponExit.position ), Vector3.up );
								...
								targetAimLocation = weaponExit.position + ( _eleAngleDir * Vector3.Cross( _cross, Vector3.up ) ).normalized * Vector3.Distance( weaponExit.position, targetLoc );
							}
						}
						if ( hasSolution == false ) { // no solution for ballistic aim, aim 45° opposite gravity
							Vector3 _cross = -Vector3.Cross( ( targetLoc - weaponExit.position ), Vector3.up );
							Quaternion _eleAngleDir = Quaternion.AngleAxis( _factor * 45f, -_cross );
							targetAimLocation = weaponExit.position + ( _eleAngleDir * Vector3.Cross( _cross, Vector3.up ) ).normalized * Vector3.Distance( weaponExit.position, targetLoc );
						}
		...
		void TurnSound ( AudioObject ao, float rate, float rateMax ) {
			if ( rate > .01f ) { // prevent near 0 from floating point error
		...
		void CheckGimbalLimits () {
			...
			if ( rotator.localEulerAngles.y < 360 - limitLeftRight && rotator.localEulerAngles.y >= 180 ) {
