...
namespace AmplifyShaderEditor
{
	...
	public sealed class AmplifyImpostorNode : ParentNode
	{
		...
		public override string GenerateShaderForOutput( int outputId, ref MasterNodeDataCollector dataCollector, bool ignoreLocalvar )
		{
			...
			if( m_customImpostorType == CustomImpostorType.Spherical )
				functionResult = dataCollector.AddFunctions( m_functionHeaderSphere, m_functionBody, vertIN + ".vertex", vertIN + ".normal", vertIN + ".texcoord", vertOut + "." + sphereFrames, vertOut + "." + viewPosName );
			else
				functionResult = dataCollector.AddFunctions( m_functionHeader, m_functionBody, vertIN+".vertex", vertIN + ".normal", vertIN + ".texcoord", vertOut + "." + uvFrame1Name, vertOut + "." + uvFrame2Name, vertOut + "." + uvFrame3Name, vertOut + "." + octaFrameName, vertOut + "." + viewPosName );
		...
		{
			m_functionBody = string.Empty;
			IOUtils.AddFunctionHeader( ref m_functionBody, "inline void SphereImpostorVertex( inout float4 vertex, inout float3 normal, inout float4 texcoord, inout float4 frameUVs, inout float4 viewPos )" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "vertex.xyz += _AI_Offset.xyz;" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "float2 uvExpansion = texcoord.xy - 0.5;" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "float2 frameUV = ( texcoord.xy + relativeCoords ) * sizeFraction;" );
		...
		private void GenerateSphereImpostorFragment( bool withSpec = true, bool withEmission = true, bool isSRP = false )
		{
			...
			IOUtils.AddFunctionHeader( ref m_functionBody, "inline void SphereImpostorFragment( inout "+ structure + " o, out float4 clipPos, out float3 worldPos, float2 frameUV, float4 viewPos" + extraHeader + " )" );
			IOUtils.AddFunctionLine( ref m_functionBody, "float4 albedoSample = tex2Dbias( _Albedo, float4( frameUV, 0, _AI_TextureBias) );" );
			IOUtils.AddFunctionLine( ref m_functionBody, "float4 normalSample = tex2Dbias( _Normals, float4( frameUV, 0, _AI_TextureBias) );" );
			if( withSpec )
				IOUtils.AddFunctionLine( ref m_functionBody, "float4 specularSample = tex2Dbias( _Specular, float4( frameUV, 0, _AI_TextureBias) );" );
			if( withEmission )
				IOUtils.AddFunctionLine( ref m_functionBody, "float4 emissionSample = tex2Dbias( _Emission, float4( frameUV, 0, _AI_TextureBias) );" );
			...
			for( int i = 0; i < m_extraSamplers; i++ )
			{
				if( GetInputPortByArrayId( i ).IsConnected && GetOutputPortByArrayId( i ).IsConnected )
				{
					IOUtils.AddFunctionLine( ref m_functionBody, "output" + i + " = tex2Dbias( " + m_extraPropertyNames[ i ] + ", float4( frameUV, 0, _AI_TextureBias) );" );
				}
			}
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "#if defined(SHADOWS_DEPTH) || ( defined(SHADERPASS) && (SHADERPASS == SHADERPASS_SHADOWS) ) || defined(AI_RENDERPIPELINE) && defined(UNITY_PASS_SHADOWCASTER)" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "#if !defined(AI_RENDERPIPELINE) && defined(UNITY_PASS_SHADOWCASTER)" );
			IOUtils.AddFunctionLine( ref m_functionBody, "clipPos = UnityApplyLinearShadowBias( clipPos );" );
			IOUtils.AddFunctionLine( ref m_functionBody, "#elif defined(AI_RENDERPIPELINE) && defined(UNITY_PASS_SHADOWCASTER) && !defined(SHADERPASS)" );
			//IOUtils.AddFunctionLine( ref m_functionBody, "clipPos.z += _ShadowBias.x;" );
		...
		{
			m_functionBody = string.Empty;
			IOUtils.AddFunctionHeader( ref m_functionBody, "inline void OctaImpostorVertex( inout float4 vertex, inout float3 normal, inout float4 texcoord, inout float4 uvsFrame1, inout float4 uvsFrame2, inout float4 uvsFrame3, inout float4 octaFrame, inout float4 viewPos )" );
			...
			IOUtils.AddFunctionLine( ref m_functionBody, "float3 objectHorizontalVector = normalize( cross( objectCameraDirection, upVector ) );" );
			IOUtils.AddFunctionLine( ref m_functionBody, "float3 objectVerticalVector = cross( objectHorizontalVector, objectCameraDirection );" );
			IOUtils.AddFunctionLine( ref m_functionBody, "float2 uvExpansion = ( texcoord.xy - 0.5f ) * UVscale;" );
