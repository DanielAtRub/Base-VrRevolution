...
public class AimAnimIndioScript : NetworkBehaviour
{
    [SyncVar]
    ...
    void OnAnimatorIK()
    {
        ...
        if (ikActive)
        {
            if (isServerOnly)
            {
                if (behavior.GetComponent<MobFarm.MF_DemoGun>().N_target)
                    target = behavior.GetComponent<MobFarm.MF_DemoGun>().N_target.transform.position;
            }
